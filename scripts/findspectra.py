...
def main():
    ...
    argparser.add_argument('--tolerance', action='store', type=float, help='Tolerance of a specified peak m/z and a measured peak m/z')
    ...
    if params.tolerance is None or params.tolerance == 0.0:
        params.tolerance = 0.01
    print(f"tolerance is {params.tolerance}")

    if (hasMZCheck):
        lowerBound = params.precursor_mz - params.tolerance
        upperBound = params.precursor_mz + params.tolerance
                ...
                if (needToCheckPeaks):
                    for peak in requiredPeaksList:
                        if satisfyRequirements:
                            satisfyRequirements = peak in spectrum['intensity array']
                        else:
                            break
                    if satisfyRequirements:
